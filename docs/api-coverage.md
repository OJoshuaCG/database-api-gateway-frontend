...
| 52 | `DELETE .../migrations/{version}` | ✅ | Desde el pie de `VersionFactsCard` (antes, el pie del card de SQL). Habilitado según `deletable` del backend, con el motivo del `block_reason` **como texto visible** y no como `title` de un `<span>` —que no llega por teclado ni en táctil—, y con doble confirmación (`confirmWord`: hay que reescribir el número de versión), porque al subir el botón se perdió la fricción de bajar por todo el SQL. Se deshabilita también si el detalle de la versión no cargó: puede haberse borrado por debajo. El 409 se sigue explicando en línea |
