        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-col gap-1">
            <h2 className="text-sm font-semibold text-foreground">
              Reconciliar aplicación parcial
            </h2>
            <p className="text-xs text-muted-foreground">
              Migración{' '}
              <code className="rounded bg-surface-muted px-1.5 py-0.5">{entry.version}</code>
            </p>
          </div>
          <IconButton
            label="Cerrar"
        ...
        {executed ? (
          ...
          <>
            <p className="text-sm text-muted-foreground">
              ...
              <strong>{entry.total_statements}</strong> sentencias antes de fallar. La
              reconciliación ejecuta los reversos de las sentencias aplicadas (de la última a la
              primera) y limpia el registro parcial, devolviendo la BD a la versión anterior.
            </p>
                ...
                {requiresForce && (
                  <div className="flex flex-col gap-2 rounded-lg border border-warning/40 bg-warning/5 p-3 text-xs">
                    <p className="text-foreground">
                      <strong>
                        {plan.unreversible_statements.length} sentencia(s) aplicadas no tienen
                        reverso conocido
                      </strong>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {result.results.length > 0 && (
        <ul className="flex max-h-96 flex-col divide-y divide-border overflow-auto rounded-lg border border-border">
